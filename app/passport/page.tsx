...
import { FaGithub, FaGoogle, FaTwitter, FaTrophy, FaUser, FaWallet, FaLink, FaCalendar, FaRocket, FaShieldAlt } from "react-icons/fa";
...
function WalletConnectPrompt() {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="bg-white dark:bg-black rounded-3xl shadow-2xl p-12 border border-gray-200 dark:border-gray-800 text-center">
                <div className="mb-8">
...
function PassportOverview({ userData, account, totalScore, walletLinks }: {
    userData: UserData;
    account: any;
    totalScore: number;
    ...
    return (
        <div className="bg-background rounded-3xl shadow-2xl p-8 border border-gray-200 dark:border-gray-800">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                            {userData.primaryWallet ? `${userData.primaryWallet.slice(0, 6)}...${userData.primaryWallet.slice(-4)}` : "No wallet provided"}
                        </p>
                    </div>
                </div>
                            ...
                            {walletLinks.map((wallet, index) => (
                                <div key={index} className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-700">
                                    <div className="flex items-center space-x-3">
                                        <div className={`w-3 h-3 rounded-full ${wallet.isPrimary ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                                        <div>
...
function SocialConnections({
    ...
    return (
        <div className="bg-background rounded-3xl shadow-xl p-6 border border-gray-200 dark:border-gray-800">
            ...
            {socialLinks.length > 0 ? (
                        ...
                        {socialLinks.map((link, index) => (
                            ...
                            >
                                ...
                                <div className="flex items-center space-x-4">
                                    ...
                                    <button
                                        onClick={() => handleSocialUnlink(link.provider as "google" | "twitter" | "github")}
                ...
                <div className="mb-6 text-center py-8">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 text-sm max-w-md mx-auto">
                        Link your Google, Twitter, and GitHub accounts to build your digital identity and earn reputation scores.
                    </p>
                </div>
            )}
                ...
                <div className="space-y-3">
                    {["google", "twitter", "github"]
                        ...
                        .map((provider) => (
                            <button
                                key={provider}
                                onClick={() => handleSocialLink(provider as "google" | "twitter" | "github")}
                                disabled={loading !== null}
                    ...
                    {["google", "twitter", "github"].filter(provider => !isProviderLinked(provider)).length === 0 && (
                    ...
                    {socialLinks.length === 0 && ["google", "twitter", "github"].filter(provider => !isProviderLinked(provider)).length > 0 && (
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center space-x-2">
                                    <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 text-xs font-medium rounded-full">
                                        {typeof entry.badge === 'string' ? entry.badge.replace('_', ' ').toUpperCase() : 'BADGE'}
                                    </span>
...
export default function PassportPage() {
    const { account, connected, handleWalletLogin, authenticated, userData: walletUserData } = useWallet();
    ...
    }, [connected, account, authenticated, autoLoginTriggered, handleWalletLogin]);
    ...
    useEffect(() => {
        ...
        if ((authenticated && accessToken) || (!authenticated && accessToken && connected)) {
    ...
    useEffect(() => {
        ...
        if (socialLinks.length > 0 && loading) {
            ...
            if (socialLinks.some(link => link.provider === loadingProvider)) {
                console.log(`Clearing loading state for ${loadingProvider} - connection detected`);
                setLoading(null);
            }
        }
    }, [socialLinks, loading]);
    ...
    const handleSocialLink = async (provider: "google" | "twitter" | "github") => {
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            console.log("Origin check:", { origin: event.origin, allowed: allowedOrigins, isAllowed: isAllowedOrigin });
            ...
            if (isAllowedOrigin && event.data) {
                ...
                if (typeof event.data === "string") {
                    try {
                        ...
                        responseData = { success: successStrings.includes(event.data.toLowerCase()) };
                    }
                }
                ...
                if (typeof responseData === "object" && responseData !== null) {
                    ...
                    const isOAuthMessage =
                        ...
                        (typeof responseData === "string" &&
                            ["success", "connected", "linked", "error", "failed"].some(keyword =>
                    ...
                    const isSuccess = responseData.success === true ||
                        ...
                        (responseData.status === 200 || responseData.statusCode === 200) ||
                        // If no explicit error and has user data, consider it a success
                        (!responseData.error && !responseData.message?.includes("error") &&
                            (responseData.socialId || responseData.userId || responseData.id || responseData.email));
                    ...
                    if (isSuccess) {
                        ...
                        const newSocialLink: SocialLink = {
                            provider: provider,
                            socialId: responseData.socialId || responseData.userId || responseData.id || `${provider}_${Date.now()}`,
                            ...
                            profileUrl: responseData.profileUrl || responseData.html_url || '',
                        ...
                        toast({
                            title: "Success",
                            description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} account linked successfully`,
                        });
        ...
        try {
            ...
            if (socialLinks.some(link => link.provider === provider)) {
                toast({
                    ...
                    description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} account is already linked`,
                });
                setLoading(null);
                return;
            }
            ...
            const urlWithToken = `${authUrl}?accessToken=${encodeURIComponent(accessToken)}&origin=${encodeURIComponent(window.location.origin)}`;
            ...
            const wasLinkedInitially = socialLinks.some(link => link.provider === provider);
            ...
            dataRefreshInterval = setInterval(async () => {
                ...
                try {
                    ...
                    const isNowLinked = socialLinks.some(link => link.provider === provider);
            ...
            checkPopupInterval = setInterval(() => {
                if (popupWindow?.closed) {
                    ...
                    setTimeout(async () => {
                        ...
                        const wasAlreadyLinked = socialLinks.some(link => link.provider === provider);

                        try {
                            console.log("Fetching updated passport data to check connection status...");
                            await fetchPassportData();
                            ...
                            setTimeout(() => {
                                ...
                                if (!wasAlreadyLinked && isNowLinked) {
                                    console.log("Connection successful - detected via data refresh");
                                    toast({
                                        title: "Success",
                                        description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} account linked successfully`,
                                    });
                                    ...
                                    toast({
                                        title: "Cancelled",
                                        description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} linking was cancelled or failed`,
                                    });
                                }
                            ...
                            if (loading === provider) {
                                toast({
                                    title: "Cancelled",
                                    description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} linking was cancelled`,
                                });
                            }
                        }
            ...
            messageTimeout = setTimeout(() => {
                ...
                toast({
                    variant: "destructive",
                    title: "Timeout",
                    description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} linking timed out. Please try again.`,
                });
            }, 5 * 60 * 1000); // 5 minutes
            ...
            toast({
                variant: "destructive",
                title: "Error",
                description: error instanceof Error ? error.message : `Failed to link ${provider} account`,
            });
            cleanup();
        }
    };

    const handleSocialUnlink = async (provider: "google" | "twitter" | "github") => {
        ...
        try {
            ...
            if (isSuccess) {
                ...
                setSocialLinks(prev => prev.filter(link => link.provider !== provider));

                toast({
                    title: "Success",
                    description: `${provider.charAt(0).toUpperCase() + provider.slice(1)} account unlinked successfully`,
                });
            ...
            toast({
                variant: "destructive",
                title: "Error",
                description: error instanceof Error ? error.message : `Failed to unlink ${provider} account`,
            });
        } finally {
            setLoading(null);
        }
    ...
    const getTotalScore = () => {
        ...
        return scoreHistory.reduce((total: number, entry: ScoreEntry) => total + entry.totalScore, 0);
    };
            ...
            {!connected ? (
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    <div className="text-center py-12">
                        <div className="max-w-md mx-auto bg-background rounded-2xl shadow-xl p-8 border border-gray-700">
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
                                ...
                                <div className="relative">
                                    <div className="bg-gray-800 border border-orange-500/30 rounded-2xl p-8 shadow-xl">
                                        <div className="flex items-center gap-4 mb-6">
                                            ...
                                            </div>                                        <h2 className="text-xl font-semibold text-teal-400">Unique Humanity Score</h2>
                                        </div>
                                ...
                                <div className="flex-1 lg:ml-12">
                                    <h1 className="text-4xl font-bold mb-4">Let's increase that score</h1>
                                    <a href="#" className="text-teal-400 underline text-lg hover:text-teal-300 transition-colors">
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                            ...
                            <div className="mb-12">
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        ...
                                        <p className="text-gray-400">
                                            Connect your blockchain-based profiles and assets to prove your identity.
                                        </p>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-bold text-teal-400">22.5</div>
                                                <div className="text-sm text-gray-300">Available Points</div>
                                            </div>
                                        </div>
                                        <h3 className="text-xl font-bold mb-2">Ethereum</h3>
                                        <p className="text-sm text-gray-300 mb-4">Verify Ethereum activity.</p>
                                        <button
                                            disabled={true}
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-bold text-teal-400">22.1</div>
                                                <div className="text-sm text-gray-300">Available Points</div>
                                            </div>
                                        </div>
                                        <h3 className="text-xl font-bold mb-2">NFT</h3>
                                        <p className="text-sm text-gray-300 mb-4">Connect your Ethereum wallet to verify your Ethereum Mainnet NFTs.</p>
                                        <button
                                            disabled={true}
                                            className="w-full bg-gray-600 text-gray-400 py-3 rounded-xl font-semibold transition-colors cursor-not-allowed"
                                        >
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        ...
                                        <p className="text-sm text-gray-300 mb-4">Stake GTC to boost your trust in the Gitcoin ecosystem.</p>
                                        <button
                                            disabled={true}
                                            className="w-full bg-gray-600 text-gray-400 py-3 rounded-xl font-semibold transition-colors cursor-not-allowed"
                                        >
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-bold text-teal-400">9.7</div>
                                                <div className="text-sm text-gray-300">Available Points</div>
                                            </div>
                                        </div>
                                        <h3 className="text-xl font-bold mb-2">Idena</h3>
                                        <p className="text-sm text-gray-300 mb-4">Prove Your Unique Humanity with Idena</p>
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-bold text-teal-400">6.2</div>
                                                <div className="text-sm text-gray-300">Available Points</div>
                                            </div>
                                        </div>
                                        <h3 className="text-xl font-bold mb-2">Gitcoin</h3>
                                        <p className="text-sm text-gray-300 mb-4">Verify your participation in Gitcoin Grants rounds.</p>
                                        <button className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold transition-colors">
                                            Connect
                                        </button>
                                    </div>
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-bold text-teal-400">1.5</div>
                                                <div className="text-sm text-gray-300">Available Points</div>
                                            </div>
                                        </div>
                                        <h3 className="text-xl font-bold mb-2">Phone Verification</h3>
                                        <p className="text-sm text-gray-300 mb-4">Connect to Zeronym by Holonym to verify your phone number.</p>
                                        <button className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold transition-colors">
                                            Connect
                                        </button>
                                    </div>
                                    ...
                                    <div className="hover-grid bg-background border border-blue-700 rounded-2xl p-6 text-white transition-all duration-300 hover:border-blue-500 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <div className="text-right">
                                                <div className="text-2xl font-bold text-teal-400">0.7</div>
                                                <div className="text-sm text-gray-300">Available Points</div>
                                            </div>
                                        </div>
                                        <h3 className="text-xl font-bold mb-2">Guild Membership and Roles</h3>
                                        <p className="text-sm text-gray-300 mb-4">Connect to Guild to verify your membership in open source communities.</p>
                                        <button className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-semibold transition-colors">
                                            Connect
                                        </button>
                                    </div>
                            ...
                            <div className="mb-12">
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        ...
                                        <p className="text-teal-400">
                                            Link your profiles from established social media and professional networking sites for verification.
                                        </p>
                                    </div>
                                    <button className="text-gray-400 hover:text-gray-300">
                                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    {/* LinkedIn */}
                                    <div className="hover-grid bg-background border border-gray-600 rounded-2xl p-6 text-white transition-all duration-300 hover:border-gray-400 cursor-pointer">
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center">
                                                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                                        ...
                                        <p className="text-sm text-gray-300 mb-4">This stamp confirms that your LinkedIn account is verified and includes a valid, verified email address.</p>
                                    ...
                                    <div className="hover-grid bg-background border border-gray-600 rounded-2xl p-6 text-white transition-all duration-300 hover:border-gray-400 cursor-pointer">
                                        ...
                                        <p className="text-sm text-gray-300 mb-4">Connect your Discord account to Passport to identity and reputation in Web3 communities.</p>
                                        <button className="w-full bg-gray-600 hover:bg-gray-500 text-white py-3 rounded-xl font-semibold transition-colors">
                                            Coming Soon
                                        </button>
                                    </div>
                                    ...
                                    {/* Twitter */}
                                    <div className={`hover-grid ${socialLinks.some(link => link.provider === 'twitter') ? 'bg-background border-green-700 hover:border-green-500' : 'bg-background border-blue-700 hover:border-blue-500'} rounded-2xl p-6 text-white transition-all duration-300 cursor-pointer relative`}>
                                        ...
                                        <div className="flex items-center justify-between mb-4">
                                            <div className={`w-12 h-12 ${socialLinks.some(link => link.provider === 'twitter') ? 'bg-green-600' : 'bg-blue-600'} rounded-xl flex items-center justify-center`}>
                                                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                                            ...
                                            <div className="text-right">
                                                {socialLinks.some(link => link.provider === 'twitter') ? (
                                                    <div>
                                                        ...
                                                        <div className="text-2xl font-bold text-teal-400">2.5</div>
                                        ...
                                        {socialLinks.some(link => link.provider === 'twitter') ? (
                                            <div className="text-sm text-gray-300 mb-4">
                                                <div className="flex items-center mb-2">
                                                    <svg className="w-4 h-4 mr-2 text-teal-400" viewBox="0 0 24 24" fill="currentColor">
                                            ...
                                            <p className="text-sm text-gray-300 mb-4">Connect your Twitter account to verify your social presence.</p>
                                        )}
                                        <button
                                            onClick={() => socialLinks.some(link => link.provider === 'twitter') ? handleSocialUnlink("twitter") : handleSocialLink("twitter")}
                                            disabled={loading === "twitter"}
                                            className={`w-full ${socialLinks.some(link => link.provider === 'twitter') ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'} text-white py-3 rounded-xl font-semibold transition-colors disabled:opacity-50`}
                                        >
                                            {loading === "twitter" ? (
                                                <div className="flex items-center justify-center">
                                                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                                                    {socialLinks.some(link => link.provider === 'twitter') ? 'Disconnecting...' : 'Connecting...'}
                                                </div>
                                            ) : (
                                                socialLinks.some(link => link.provider === 'twitter') ? 'Disconnect' : 'Connect'
                                            )}
                                        </button>
                                    </div>

                                    {/* GitHub - Blue-black when not connected, green-black when connected */}
                                    <div className={`hover-grid ${socialLinks.some(link => link.provider === 'github') ? 'bg-background border-green-700 hover:border-green-500' : 'bg-background border-blue-700 hover:border-blue-500'} rounded-2xl p-6 text-white transition-all duration-300 cursor-pointer relative`}>
                                        ...
                                        <div className="flex items-center justify-between mb-4">
                                            <div className={`w-12 h-12 ${socialLinks.some(link => link.provider === 'github') ? 'bg-green-600' : 'bg-blue-600'} rounded-xl flex items-center justify-center`}>
                                                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                                            ...
                                            <div className="text-right">
                                                {socialLinks.some(link => link.provider === 'github') ? (
                                                    <div>
                                                        <div className="text-lg font-bold text-white mb-1">Points gained</div>
                                                        <div className="text-2xl font-bold text-teal-400">3.8</div>
                                                        <div className="w-16 h-2 bg-gray-600 rounded-full mt-2">
                                        ...
                                        {socialLinks.some(link => link.provider === 'github') ? (
                                            ...
                                            <p className="text-sm text-gray-300 mb-4">Connect your GitHub account to verify your development activity.</p>
                                        )}
                                        <button
                                            onClick={() => socialLinks.some(link => link.provider === 'github') ? handleSocialUnlink("github") : handleSocialLink("github")}
                                            disabled={loading === "github"}
                                            className={`w-full ${socialLinks.some(link => link.provider === 'github') ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'} text-white py-3 rounded-xl font-semibold transition-colors disabled:opacity-50`}
                                        >
                                            {loading === "github" ? (
                                                <div className="flex items-center justify-center">
                                                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                                                    {socialLinks.some(link => link.provider === 'github') ? 'Disconnecting...' : 'Connecting...'}
                                                </div>
                                            ) : (
                                                socialLinks.some(link => link.provider === 'github') ? 'Disconnect' : 'Connect'
                                            )}
                                        </button>
                                    </div>

                                    {/* Google - Blue-black when not connected, green-black when connected */}
                                    <div className={`hover-grid ${socialLinks.some(link => link.provider === 'google') ? 'bg-background border-green-700 hover:border-green-500' : 'bg-background border-blue-700 hover:border-blue-500'} rounded-2xl p-6 text-white transition-all duration-300 cursor-pointer relative`}>
                                        ...
                                        <div className="flex items-center justify-between mb-4">
                                            <div className={`w-12 h-12 ${socialLinks.some(link => link.provider === 'google') ? 'bg-green-600' : 'bg-blue-600'} rounded-xl flex items-center justify-center`}>
                                                <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                                        ...
                                        {socialLinks.some(link => link.provider === 'google') ? (
                                            ...
                                            <p className="text-sm text-gray-300 mb-4">Connect your Google account for identity verification.</p>
                                        )}
                                        <button
                                            onClick={() => socialLinks.some(link => link.provider === 'google') ? handleSocialUnlink("google") : handleSocialLink("google")}
                                            disabled={loading === "google"}
                                            className={`w-full ${socialLinks.some(link => link.provider === 'google') ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'} text-white py-3 rounded-xl font-semibold transition-colors disabled:opacity-50`}
                                        >
                                            {loading === "google" ? (
                                                <div className="flex items-center justify-center">
                                                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>
                                                    {socialLinks.some(link => link.provider === 'google') ? 'Disconnecting...' : 'Connecting...'}
                                                </div>
                                            ) : (
                                                socialLinks.some(link => link.provider === 'google') ? 'Disconnect' : 'Connect'
                                            )}
                                        </button>
                                    </div>
                                </div>
