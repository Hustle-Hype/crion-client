...
const RippleGrid: React.FC<Props> = ({
    enableRainbow = false,
    gridColor = "#ffffff",
    rippleIntensity = 0.04, // giảm ripple
    gridSize = 8.0, // lưới rộng hơn
    gridThickness = 13.0,
    fadeDistance = 1.5,
...
void main() {
    ...
    float func = sin(pi * (iTime * 0.7 - dist)); // chuyển động chậm lại
    ...
    return (
        <div
            ref={containerRef}
            className="absolute inset-0 w-full h-full z-0 overflow-hidden pointer-events-none [&_canvas]:block"
        />
    );
};
