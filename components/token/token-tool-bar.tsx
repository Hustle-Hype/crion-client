...
function DropdownSortBy({ sortBy, setSortBy }: { sortBy: string; setSortBy: (val: string) => void }) {
    ...
    return (
        <div ref={ref} className="relative w-max min-w-[220px]">
            <button
                type="button"
                className={`group flex items-center w-full h-[48px] min-h-[48px] rounded-full px-5 pr-10 bg-[#181818] focus:outline-none relative transition-colors border border-transparent ${!open ? 'hover:border-[#2D6BFF]' : ''}`}
            ...
            >
                ...
                <span className="mr-2 opacity-60">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><g><path d="M10 4.5V15.5" stroke="#707472" strokeWidth="1.5" strokeLinecap="round" /><path d="M7 7.5L10 4.5L13 7.5" stroke="#707472" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M13 12.5L10 15.5L7 12.5" stroke="#707472" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></g></svg>
                </span>
                <span className="text-[15px] font-medium text-[#707472] mr-1">Sort by:</span>
                <span className="text-[16px] font-semibold text-white ml-1">{sortOptions.find(o => o.value === sortBy)?.label || 'Sort'}</span>
                {/* Chevron icon */}
                <span className={`absolute right-4 w-5 h-5 flex items-center transition-transform duration-150 ${open ? 'rotate-180' : ''}`}>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M7 8L10 11L13 8" stroke="#707472" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                </span>
            </button>
            {open && (
                <ul
                    className="absolute z-50 top-[52px] right-0 bg-[#181818] rounded-2xl shadow-lg py-1 mt-1  min-w-[160px] w-[160px]"
                    role="listbox"
                >
                    {sortOptions.map(opt => (
                        <li
                            ...
                            className={`px-4 py-1.5 text-[15px] font-semibold cursor-pointer transition-colors rounded-lg ${sortBy === opt.value ? 'bg-[#232F47] text-[#2D6BFF]' : 'text-white hover:bg-[#2D6BFF] hover:bg-[#232F47]/80 hover:px-5'}`}
                            style={{ margin: '2px 0' }}
