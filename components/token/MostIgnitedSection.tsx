...
export default function MostIgnitedSection({ tokens = [] }: { tokens: TokenInfo[] }) {
    ...
    return (
        <section className="w-full flex flex-col md:flex-row gap-8 md:gap-0 py-20 px-4 md:px-20 bg-[#121316] min-h-[600px]">
                    ...
                    {tokens.slice(start, end).map((token, idx) => {
                        ...
                        return (
                            <a
                                ...
                                className="flex flex-col z-10 rounded-xl transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-blue-500/20 hover:-translate-y-2 cursor-pointer borderToken min-w-[313px] max-w-[340px] w-full"
                                style={{ minHeight: 367 }}
                            >
